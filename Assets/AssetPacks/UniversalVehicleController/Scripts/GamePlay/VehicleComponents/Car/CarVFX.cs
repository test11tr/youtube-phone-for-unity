...
namespace PG
{
    ...
    public class CarVFX :VehicleVFX
    {
        ...
        protected override void Awake ()
        {
            ...
            Car.OnStartEngineAction += OnStartEngine;
        }
        ...
        void OnStartEngine (float startEngineDellay)
        {
            if (ExhaustParticles.Count > 0)
            {
                StartCoroutine (OnStartEngineCoroutine(startEngineDellay));
            }
        }

        IEnumerator OnStartEngineCoroutine (float startEngineDellay)
        {
            yield return new WaitForSeconds (startEngineDellay * 0.5f);

            float timer = startEngineDellay * 0.5f;
