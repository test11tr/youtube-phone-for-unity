...
namespace PG
{
    ...
    public class AIPathEditor :Editor
    {
        ...
        float LastAddPointTime;

        const string ControlHelpInfo = "\nTip: Lock the inspector (Lock icon on the top right side of the inspector) to make it easier to edit points.\n\n'Shift + LMB' To create a point after the selected point at the cursor position.\n\n'Ctrl + LMB' to move the selected point to the cursor position.\n";
        ...
        public override void OnInspectorGUI ()
        {
            ...
            EditorGUILayout.HelpBox (ControlHelpInfo, MessageType.Info);
        }

        void AddWaypointInMousePos (int index = -1)
        {
            ...
            if (Physics.Raycast (ray, out hit, 2000))
            {
                ...
                AIPath.WaypointData point;
                AIPath.WaypointData prevPoint;

                if (index == -1)
                {
                    point = AIPath.Waypoints[AIPath.Waypoints.Count - 1];
                    prevPoint = AIPath.Waypoints[AIPath.Waypoints.Count - 2];
                }
                else
                {
                    point = AIPath.Waypoints[index + 1];
                    prevPoint = AIPath.Waypoints[index];
                }

                point.Point.position = hit.point + hit.normal;
                point.Point.rotation = Quaternion.LookRotation (point.Point.position - prevPoint.Point.position, hit.normal);

                UpdateList ();
            }
        }

        void MoveSelectedWaypointInMousePos (int index)
        {
            Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
            RaycastHit hit;
            ...
            if (Physics.Raycast (ray, out hit, 2000))
            {
                ...
                if (index == -1)
                {
                    index = AIPath.Waypoints.Count - 1;
                }

                point = AIPath.Waypoints[index];
                point.Point.position = hit.point + hit.normal;

                if (index > 0)
                {
                    point.Point.rotation = Quaternion.LookRotation (point.Point.position - AIPath.Waypoints[index - 1].Point.position, hit.normal);
                }
        ...
        private void OnSceneGUI ()
        {
            ...
            if (e.button == 0)
            {
                if (e.type == EventType.MouseDown && Time.time - LastAddPointTime > 0.2f)
                {
                    LastAddPointTime = Time.time;

                    if (e.shift)
                    {
                        AddWaypointInMousePos (AIPath.SelectedWaypointIndex);
                        return;
                    }
                    else if (e.control && AIPath.SelectedWaypointIndex >= 0)
                    {
                        MoveSelectedWaypointInMousePos (AIPath.SelectedWaypointIndex);
                        return;
                    }
                }
            }

            if (e.button == 0 && e.clickCount > 0 && HighlightedIndex >= 0 && Waypoints[HighlightedIndex].Point != null)
            {
        ...
        void UpdateList ()
        {
            ...
            for (int i = 0; i < AIPath.Waypoints.Count; i++)
            {
                ...
                if (i < AIPath.Waypoints.Count - 1)
                {
                    nextPos = AIPath.Waypoints[i + 1].Point.position;
                }
                else if (AIPath.LoopedPath)
                {
                    nextPos = AIPath.Waypoints[0].Point.position;
                }
                else
                {
                    nextPos = AIPath.Waypoints[i].Point.position;
                }

                if ((nextPos - AIPath.Waypoints[i].Point.position).sqrMagnitude > 0.01f)
                {
                    AIPath.Waypoints[i].Point.rotation = Quaternion.LookRotation (nextPos - AIPath.Waypoints[i].Point.position, AIPath.Waypoints[i].Point.up);
                }
            }
        }
