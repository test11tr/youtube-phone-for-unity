...
#pragma warning disable 0649
        ...
        [SerializeField] GroundType DefaultType = GroundType.Asphalt;
...
#pragma warning restore 0649
        ...
        Dictionary<byte, GroundType> GroundConfigsDictionary = new Dictionary<byte, GroundType>();  //A dictionary created in Awake to find the desired texture.
        ...
        public override GroundConfig GetGroundConfig (Vector3 position)
        {
            ...
            GroundType result = GroundType.Default;

            if (!GroundConfigsDictionary.TryGetValue (textureIndex, out result))
            {
                return GroundDetection.GetGroundConfig (DefaultType);
            }

            return GroundDetection.GetGroundConfig (result);
        }
        ...
        void Awake ()
        {
            ...
            foreach (var config in GroundConfigs)
            {
                ...
                {
                    GroundConfigsDictionary.Add (config.TextureIndex, config.GroundType);
                }
            }
